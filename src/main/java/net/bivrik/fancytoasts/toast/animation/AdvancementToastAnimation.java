...
public abstract class AdvancementToastAnimation {
    ...
    public void drawIcon(DrawContext context) {
        context.drawTexture(getSetup().texture(), 68, 0, getSetup().uv()[1].getU(), getSetup().uv()[1].getV(), 26, 26, 256, 256);
        context.drawItemWithoutEntity(getSetup().display().getIcon(), 73, 5);
    }
    public void drawBanner(DrawContext context) {
        context.drawTexture(getSetup().texture(), 0, 5, getSetup().uv()[0].getU(), getSetup().uv()[0].getV(), 162, 14, 256, 256);
    }
    public void drawBackground(DrawContext context) {
        context.drawTexture(getSetup().texture(), 0, 20, 0, 0, 162, 40, 256, 256);
        context.drawTexture(getSetup().texture(), 144, 56, 0, 108, 9, 14, 256, 256);
    }
